...
# extract data
Y <- t(exprs(pcaHHgse6919a))
stage <- as.numeric(pData(pcaHHgse6919a)[,1])
...
gammaObs <- rbind(c("gse6919a", sum(stage==0), sum(stage==1), ncol(Y), "no", "no", "n.a.", median(optLambdaCweak), 
			diffGGMml(covML(Y[which(stage==0),]), covML(Y[which(stage==1),]), sum(stage==0), sum(stage==1), lambda=median(optLambdaCweak), Uequal=TRUE)$gamma, NA), 
		   	c("gse6919a", sum(stage==0), sum(stage==1), ncol(Y), "yes", "no", "n.a.", median(optLambdaCstrong), 
...
# estimate gamma (= observed test statistic)
gammaObs <- rbind(gammaObs, c("gse6919a", sum(stage==0), sum(stage==1), ncol(Y), "no", "yes", "n.a.", median(optLambdaCweak), 
			diffGGMml(covML(Y[which(stage==0),]), covML(Y[which(stage==1),]), sum(stage==0), sum(stage==1), lambda=median(optLambdaCweak), Uequal=TRUE)$gamma, NA), 
		   	c("gse6919a", sum(stage==0), sum(stage==1), ncol(Y), "yes", "yes", "n.a.", median(optLambdaCstrong), 
			diffGGMml(covML(Y[which(stage==1),]), covML(Y[which(stage==0),]), sum(stage==1), sum(stage==0), lambda=median(optLambdaCstrong), Uequal=TRUE)$gamma, NA))
...
write.table(gammaObs, quote=FALSE, row.names=TRUE, col.names=TRUE, sep="\t", file="results_pcaHHgse6919a.txt")
