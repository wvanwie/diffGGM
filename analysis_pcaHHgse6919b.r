...
# READ ME
#
# Analysis hedgehog signalling pathway data from the GEO prostate cancer study GSE6919b.
...
# extract data
Y <- t(exprs(pcaHHgse6919b))
stage <- as.numeric(pData(pcaHHgse6919b)[,1])
...
gammaObs <- rbind(c("gse6919b", sum(stage==0), sum(stage==1), ncol(Y), "no", "no", "yes", median(optLambdaCweak), 
		   ...
		   c("gse6919b", sum(stage==0), sum(stage==1), ncol(Y), "yes", "no", "yes", median(optLambdaCstrong), 
...
# estimate gamma (= observed test statistic)
gammaObs <- rbind(gammaObs, c("gse6919b", sum(stage==0), sum(stage==1), ncol(Y), "no", "yes", "yes", median(optLambdaCweak), 
			diffGGMml(covML(Y[which(stage==0),]), covML(Y[which(stage==1),]), sum(stage==0), sum(stage==1), lambda=median(optLambdaCweak), Uequal=TRUE)$gamma, NA), 
		   	c("gse6919b", sum(stage==0), sum(stage==1), ncol(Y), "yes", "yes", "yes", median(optLambdaCstrong), 
			diffGGMml(covML(Y[which(stage==1),]), covML(Y[which(stage==0),]), sum(stage==1), sum(stage==0), lambda=median(optLambdaCstrong), Uequal=TRUE)$gamma, NA))
...
write.table(gammaObs, quote=FALSE, row.names=TRUE, col.names=TRUE, sep="\t", file="results_pcaHHgse6919b.txt")


#########################################
# same analysis with Gaussianized data without outlier
#########################################
...
# estimate gamma (= observed test statistic)
gammaObs <- rbind(gammaObs, c("gse6919b", sum(stage==0), sum(stage==1), ncol(Y), "no", "no", "no", median(optLambdaCweak), 
			diffGGMml(covML(Y[which(stage==0),]), covML(Y[which(stage==1),]), sum(stage==0), sum(stage==1), lambda=median(optLambdaCweak), Uequal=TRUE)$gamma, NA), 
		   c("gse6919b", sum(stage==0), sum(stage==1), ncol(Y), "yes", "no", "no", median(optLambdaCstrong), 
			diffGGMml(covML(Y[which(stage==1),]), covML(Y[which(stage==0),]), sum(stage==1), sum(stage==0), lambda=median(optLambdaCstrong), Uequal=TRUE)$gamma, NA))
colnames(gammaObs) <- c("dataset", "n1", "n2", "p", "c>n", "gauss", "outlier", "lambda", "gamma", "p-value")
...
#########################################
# same analysis with Gaussianized data with outlier removed
#########################################
...
# estimate gamma (= observed test statistic)
gammaObs <- rbind(gammaObs, c("gse6919b", sum(stage==0), sum(stage==1), ncol(Y), "no", "yes", "no", median(optLambdaCweak), 
			diffGGMml(covML(Y[which(stage==0),]), covML(Y[which(stage==1),]), sum(stage==0), sum(stage==1), lambda=median(optLambdaCweak), Uequal=TRUE)$gamma, NA), 
		   	c("gse6919b", sum(stage==0), sum(stage==1), ncol(Y), "yes", "yes", "no", median(optLambdaCstrong), 
			diffGGMml(covML(Y[which(stage==1),]), covML(Y[which(stage==0),]), sum(stage==1), sum(stage==0), lambda=median(optLambdaCstrong), Uequal=TRUE)$gamma, NA))
