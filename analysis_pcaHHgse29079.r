...
# READ ME
#
# Analysis hedgehog signalling pathway data from the GEO prostate cancer study GSE29079.
...
# extract data
Y <- t(exprs(pcaHHgse29079))
Y <- apply(Y, 2, function(Z){ (Z - median(Z)) })
stage <- as.numeric(pData(pcaHHgse29079)[,1])
...
gammaObs <- rbind(c("gse29079", sum(stage==0), sum(stage==1), ncol(Y), "no", "no", "n.a.", median(optLambdaCweak), 
		   ...
		   c("gse29079", sum(stage==0), sum(stage==1), ncol(Y), "yes", "no", "n.a.", median(optLambdaCstrong), 
...
# estimate gamma (= observed test statistic)
gammaObs <- rbind(gammaObs, c("gse29079", sum(stage==0), sum(stage==1), ncol(Y), "no", "yes", "n.a.", median(optLambdaCweak), 
			diffGGMml(covML(Y[which(stage==0),]), covML(Y[which(stage==1),]), sum(stage==0), sum(stage==1), lambda=median(optLambdaCweak), Uequal=TRUE)$gamma, NA), 
		        c("gse29079", sum(stage==0), sum(stage==1), ncol(Y), "yes", "yes", "n.a.", median(optLambdaCstrong), 
			diffGGMml(covML(Y[which(stage==1),]), covML(Y[which(stage==0),]), sum(stage==1), sum(stage==0), lambda=median(optLambdaCstrong), Uequal=TRUE)$gamma, NA))
...
write.table(gammaObs, quote=FALSE, row.names=TRUE, col.names=TRUE, sep="\t", file="results_pcaHHgse29079.txt")
